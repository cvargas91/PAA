...
export function reqAccionesPorRolResponsable(context, id_responsable) {
  api
    ...
    .then((response) => {
      context.commit("setTotalAcciones", response.data.length);
      context.commit("setAcciones", context.state.acciones.concat(response.data));
    })
    .catch(() => {
...
export function reqAccionesPorRolResponsableUnificado(context, rolPeriodo) {  

  api    
    .get("api/roles/rolResponsableUnificado/id_actor="+rolPeriodo.id_actor+"&anio="+rolPeriodo.anio)
    .then((response) => {
      context.commit("setTotalAcciones", response.data.length);
      context.commit("setAcciones", context.state.acciones.concat(response.data));
    })
    .catch(() => {
      console.log("error req acciones unidad Unificado resp");
    });
}

export function reqAccionesPorRolResponsableUnificadoDetalle(context, rolPeriodo) {
  api
    //.get("api/roles/rolResponsableUnificado/" + id_responsable + "/")
    .get("api/roles/rolResponsableUnificado/id_actor="+rolPeriodo.id_actor+"&anio="+rolPeriodo.anio)
    .then((response) => {
    ...
    .catch(() => {
      console.log("error req acciones unidad Unificado resp/Detalle");
    });
}
...
export function reqAccionesAReporteUnificado(context, rolPeriodo) {
  api
    //.get("api/roles/rolResponsableUnificado/" + rol + "/")
    .get("api/roles/rolResponsableUnificado/id_actor="+rolPeriodo.id_actor+"&anio="+ 0)
    .then((response) => {
      context.commit("setDetalleAccionesReporte", response.data);
      context.commit("setTotalAcciones", response.data.length);
    })
