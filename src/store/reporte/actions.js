...
export function patchReporte (context, nuevoReporte) {
    ...
    api.
        ...
        .catch((response) => {
            console.log(">>>>>error reporte/patchReporte");
        });
}
...
export function setNuevoReporteUnificado(context, nuevoReporte) {
    context.commit("setNuevoReporteUnificado", nuevoReporte);
};
...
export async function generaReporteUnificado(context, datosReportesUnificar) {    
    ...
    await api
        .get("/generarPdfUnificado/", {
            params: {
                id_reporte: datosReportesUnificar['reportes']
            },
        ...
        .then((response) => {
            ...
            const anchor = document.createElement('a')
            anchor.href = window.URL.createObjectURL(response.data)
            anchor.download = 'ReporteUnificado_Dirección_' + datosReportesUnificar['direccion'] + "__" + dia + "-" + mes + "-" + anio
            anchor.click()
            URL.revokeObjectURL(anchor.href);
        })
        .catch(() => {
            console.log("error reporte/generarPdfUnificado");
        });
}

export async function generaReporteUnificadoUnidad(context, arregloReporteUnidad) {
    let id_reporte = arregloReporteUnidad[2];
    const fechaActual = new Date();
    const dia = fechaActual.getDate().toString().padStart(2, '0');
    const mes = (fechaActual.getMonth() + 1).toString().padStart(2, '0');
    ...
    const django_token = Cookies.get("django_token");
    await api
        .get("/generarPdfUnificadoUnidad/"+ id_reporte, {
            headers: {
                "X-CSRFTOKEN": django_token,
            },
            responseType: 'blob',
        ...
        .then((response) => {
            ...
            const anchor = document.createElement('a')
            anchor.href = window.URL.createObjectURL(response.data)
            anchor.download = 'Reporte_Unificado' + arregloReporteUnidad[0] + "__" + dia + "-" + mes + "-" + anio
            anchor.click()
            URL.revokeObjectURL(anchor.href);
        })
        .catch(() => {
            console.log("error reporte/generarPdfUnificadoUnidad");
        });
}
...
export function setReporteAUnificarFinalizado (context, reportes) {
    context.commit("setReportesUnificarFinalizados", reportes);
}

export function setReporteAUnificarBorrador (context, reportes) {
    context.commit("setReportesUnificarBorradores", reportes);
}