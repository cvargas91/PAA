...
export function setNuevoReporteUnificado(state, nuevoReporte){
...
export function setActualizarReporteFinalizados(state, reporteActualizado) {
    if (state.finalizado[reporteActualizado.tipo] === null) {
        state.finalizado[reporteActualizado.tipo] = []; 
    }    
    const indiceAReemplazar = state.finalizado[reporteActualizado.tipo].findIndex(elemento => elemento.id === reporteActualizado.id);    
    state.finalizado[reporteActualizado.tipo][indiceAReemplazar] = reporteActualizado;    
};

export function setActualizarReporteBorradores(state, reporteActualizado) {    
    if (state.borrador[reporteActualizado.tipo] === null) {        
        state.borrador[reporteActualizado.tipo] = []; 
    }    

    const indiceAReemplazar = state.borrador[reporteActualizado.tipo].findIndex(elemento => {return elemento.id === reporteActualizado.id;});    
    state.borrador[reporteActualizado.tipo][indiceAReemplazar] = reporteActualizado;    
};
...
export function setReportesUnificarFinalizados (state, reportes) {
    const reportesUnificados = state.finalizado["Unificado"].filter(obj => reportes.includes(obj.actor));    
    state.reportesUnificados = reportesUnificados;
}

export function setReportesUnificarBorradores (state, reportes) {    
    const reportesUnificados = state.borrador["Unificado"].filter(obj => reportes.includes(obj.actor));
    state.reportesUnificados = reportesUnificados;
}