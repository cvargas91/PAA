<template>
    <div class="q-px-sm">        
        <q-table
            :title="origen == 'unificado' ? 'Detalle de Acciones a incluir en Reporte' : ''"
            :rows="acciones" 
            :columns="opciones"
            :selection="origen == 'Unificado' ? 'none' : 'multiple'"
        ...
        <TablaReportes
            ...
            tipoReporte="Reportes Finalizados"
        />
        
        <div class="q-pa-md" v-if="origen=='Unificado'">    
            <div class="row justify-end q-gutter-sm q-ma-md">                
                ...
                <q-btn                    
                    ...
                    label="   Reporte    "                
                    color="light-blue-10"
                    @click="reporteUnificado(acciones)"
                />
            </div>
        </div>
    ...
    setup(props) {
        ...
        return {
            ...
            verReportesFinalizados: async () => {                
                reportesFiltrados.value = reportes.value[origen].filter(elemento => elemento.actor == id_responsable.value)
                if(origen === "POA"){
            ...
            reporteUnificado: async (acciones) => {  
                accionesSeleccionadas.value = acciones
                ...
                let rolPeriodo = {
                    'id_actor' : accionesSeleccionadas.value[0].actor.id,
                    'anio' : accionesSeleccionadas.value[0].accion.anio,
                }
                //await store.dispatch("accion/reqAccionesPorRolResponsableUnificadoDetalle", accionesSeleccionadas.value[0].actor.id);
                await store.dispatch("accion/reqAccionesPorRolResponsableUnificadoDetalle", rolPeriodo);
                
                store.dispatch("reporte/setNuevoReporteUnificado", {
                    reporte: reporte,
                    acciones: accionesSeleccionadas.value
                });
                ...
                router.push("reporteUpciUnificado")
